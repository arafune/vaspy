...
assert args.poscar and args.xdatcar, "--poscar xor --xdatcar should be specified"
#
...
if args.poscar:
    for frame in range(args.frames):
        ...
        vasp_poscar.save(
            "POSCAR_mode_" + str(args.mode) + "_frame_" + str(frame) + ".vasp",
        )
else:
    ...
    for ionnum in ionnums:
        xdatcar_str += f"    {ionnum}"
    xdatcar_str += "\n"
    mat = np.linalg.inv(np.transpose(supercell))
    for frame in range(1, args.frames + 1):
        xdatcar_str += f"Direct configuration=    {frame}\n"

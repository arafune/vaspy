...
class ProjectionBand(eigenval.EnergyBand):
    ...
    def to_3dlist(self, site_indexes=(), orbital_indexes_sets=()):
        ...
        projband = np.concatenate(
            (kvalues, self.energies.T, output_proj), axis=-1)
        return projband.tolist()

    def to_csv(self, csv_file, site_indexes=(), orbital_indexes_sets=(),
               blankline=True):
        ...
        with open(csv_file, 'w') as fhandle:
            ...
            for band_i in self.to_3dlist(site_indexes,
                                         orbital_indexes_sets):
    ...
    def text_sheet(self, site_indexes=(), orbital_indexes_sets=()):
        ...
        output = '\t'.join(self.make_label(site_indexes,
                                           orbital_indexes_sets))
        output += '\n'
        list3d = self.to_3dlist(site_indexes,
                                orbital_indexes_sets)
...
class Projection(np.ndarray):
    ...
    * Igor とかで見やすいstr フォーマットを返す。←ここで定義するmethod?
    ...
    def __array_finalize__(self, viewed):
        logging.debug('In array_finalize:')
        logging.debug('   self type is %s' % type(self))
        logging.info('   self shape is {}'.format(self.shape))
        logging.debug('   viewed type is %s' % type(viewed))
        if viewed is not None:
            logging.info('   viewed shape is {}'.format(viewed.shape))
    ...
    #     logging.debug('In array_wrap:')
    #     logging.debug('   self type is %s' % type(self))
    #     logging.info( '   self shpae is {}'.format(self.shape))
    #     logging.debug('   out_arr type is %s' % type(out_arr))
    #     logging.info( '   out_arr shpae is {}'.format(out_arr.shape))
    #     arr = super(Projection, self).__array_wrap__(self, out_arr, context)
    #     logging.debug('   arr type is %s' % type(arr))
    #     logging.info( '   arr shpae is {}'.format(arr.shape))
    #     return arr
    ...
    def append_sumsite(self, sites, site_name):
        ...
        label['site'].append(site_name)
        #    spin, k, band, atom
        sumsite = self[:, :, :, sites, :].sum(axis=-2, keepdims=True)
        con_proj = np.concatenate((self, sumsite), axis=-2).view(Projection)
        con_proj.label = label
        return con_proj

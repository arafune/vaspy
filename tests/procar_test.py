...
class TestSinglePROCAR:
    ...
    def test_sigleprocar_firstcheck(self) -> None:
        """Load test for PROCAR_single."""
        assert [""] == self.singleprocar.label["spin"]
        assert self.singleprocar.n_atom == 3
        assert self.singleprocar.n_bands == 1
        assert self.singleprocar.num_k == 1
    ...
    def test_projection1(self) -> None:
        """Test Projection class."""
        # testarray is taken from PROCAR_single
        assert self.singleprocar.proj.ndim == 5
    ...
    def test_singleprocar_fermi_correction(self) -> None:
    ...
    def test_singleprocar_sum_site(self) -> None:
    ...
    def test_singleprocar_sum_orbital(self) -> None:
    ...
    def test_singleprocar_sum_site_orbital(self) -> None:
        ...
        assert len(self.singleprocar.label["site"]) == 4

    def test_singleprocar_setheader(self) -> None:
        ...
        for orbital in ("p", "pxpy", "d"):
            self.singleprocar.append_sumorbital(
                self.singleprocar.orbital_index(orbital),
                orbital,
            )
...
class TestSpinPolarizedPROCAR:
    ...
    def test_spinprocar_firstcheck(self) -> None:
        """Load test for PROCAR_spin_dummy."""
        assert ["_up", "_down"] == self.spinprocar.label["spin"]
        assert self.spinprocar.n_atom == 3
        assert self.spinprocar.n_bands == 4
        assert self.spinprocar.num_k == 3
        np.testing.assert_array_almost_equal(
            np.array(
                [
                    [
    ...
    def test_spinprocar_band(self) -> None:
        """Band_with_projection object test generated by PROCAR_spin_dummy."""
        np.testing.assert_array_almost_equal(
            self.spinprocar.kdistances,
            [0.0, 0.353553, 0.707107],
        )
    ...
    def test_spinprocar_fermi_correction(self) -> None:
    ...
    def test_spinprocar_band_orbitalread(self) -> None:
    ...
    def test_spinprocar_band_sum_orbital1(self) -> None:
        ...
        for orbital in ("p", "pxpy", "d"):
            self.spinprocar.append_sumorbital(
                self.spinprocar.orbital_index(orbital),
                orbital,
            )
        np.testing.assert_allclose(
    ...
    def test_spinprocar_make_label(self) -> None:
...
@pytest.fixture()
def soiprocar() -> procar.PROCAR:
    """Load PROCAR_SOR_dummy as test data."""
    return procar.PROCAR(str(Path(data_dir / "PROCAR_SOI_dummy")))
...
class TestSOIPROCAR:
    ...
    def test_soiprocar_firstcheck(self, soiprocar: procar.PROCAR) -> None:
        """Load test for PROCAR_SOI_dummy."""
        assert ["_mT", "_mX", "_mY", "_mZ"] == soiprocar.label["spin"]
        assert soiprocar.n_atom == 3
        assert soiprocar.n_bands == 2
        assert soiprocar.num_k == 3
        np.testing.assert_array_equal(
        ...
        np.testing.assert_array_equal(
            ["s", "py", "pz", "px", "dxy", "dyz", "dz2", "dxz", "dx2", "tot"],
            soiprocar.label["orbital"],
        )
        np.testing.assert_array_equal(
            [[0.0, 0.0, 0.0], [0.25, 0.25, 0.00], [0.50, 0.50, 0.00]],
            soiprocar.k_vectors,
        )
        np.testing.assert_array_equal(720, soiprocar.proj.size)
        np.testing.assert_array_equal(
            [
                0.0000,
                0.0001,
        ...
        np.testing.assert_array_equal(
            ...
            soiprocar.proj[0, 0, 0, 2],
        )

    def test_make_label(self, soiprocar: procar.PROCAR) -> None:
        """Test for make label from PROCAR_soi w/o append_sum*."""
        label = soiprocar.make_label((0, 2), ((0, 3, 1), (4, 7)))
        assert label == [
            "#k",
            "Energy",
            "0_mT_s",
    ...
    def test_soiprocar_band(self, soiprocar: procar.PROCAR) -> None:
        ...
        """
        soiprocar.fermi_correction(1.0)
        np.testing.assert_array_equal(
            soiprocar.energies,
            [[[-11, -6], [-8, -5], [-7, -2]]],
        )

    def test_soiprocar_band_orbitalread(self, soiprocar: procar.PROCAR) -> None:
        """Test for Band_with_projection.orbitals setter (SOI)."""
        assert soiprocar.proj.shape == (
            4,
            soiprocar.num_k,
            soiprocar.n_bands,
            soiprocar.n_atom,
    ...
    def test_soiprocar_band_sum_site(self, soiprocar: procar.PROCAR) -> None:
        """Test for Band_with_projection.sum_site (SOI)."""
        soiprocar.append_sumsite((0, 2), "test")
        assert soiprocar.label["site"].index("test") == 3
        np.testing.assert_allclose(
            soiprocar.proj[0][0][0][soiprocar.label["site"].index("test")],
            [
                0.0020,
                0.0022,
                0.0024,
    ...
    def test_soiprocar_band_sum_orbital1(self, soiprocar: procar.PROCAR) -> None:
        ...
        soiprocar.append_sumsite((0, 2), "test")
        for orbital in ("p", "pxpy", "d"):
            soiprocar.append_sumorbital(
                soiprocar.orbital_index(orbital),
                orbital,
            )
        np.testing.assert_allclose(
            soiprocar.proj[0][0][0][3],  # mT
            [
                0.0020,
                0.0022,
                0.0024,
        ...
        assert soiprocar.label["orbital"] == [
            "s",
            "py",
            "pz",
            "px",
    ...
    def test_soiprocar_make_label(self, soiprocar: procar.PROCAR) -> None:
        ...
        for orbital in ("p", "pxpy", "d"):
            soiprocar.append_sumorbital(
                soiprocar.label["orbital"].index(orbital),
                orbital,
            )
        assert soiprocar.make_label((3,), ((10, 11, 12),)) == [
            "#k",
            "Energy",
            "test_mT_p",
            "test_mX_p",
    ...
    def test_text_sheet(self, soiprocar: procar.PROCAR) -> None:
        """Test for simple band data output (SOI)."""
        assert (
            soiprocar.text_sheet()
            == """#k	Energy
...
class test_functions_in_procarpy:
    def test_tiny_check_function(self) -> None:
        ...
        assert result_single[0] == 1  # num_k
        assert result_single[1] == 1  # n_bands
        assert result_single[2] == 3  # n_atom
        assert [
            "s",
            "py",
            "pz",
        ...
        assert result_spin[0] == 3  # num_k
        assert result_spin[1] == 4  # n_bands
        assert result_spin[2] == 3  # n_atom
        assert [
            "s",
            "py",
            "pz",
        ...
        assert result_soi[0] == 3  # num_k
        assert result_soi[1] == 2  # n_bands
        assert result_soi[2] == 3  # n_atom
        assert [
            "s",
            "py",
            "pz",

...
class TestLOCPOT(object):
    ...
    def test_grid_slice_y(self):
        ...
        testarray2D = np.array([[0, 1, 2], [12, 13, 14], [24, 25, 26],
    ...
    def test_grid_slice_x(self):
        ...
        testarray2D = np.array([[0, 3, 6, 9], [12, 15, 18, 21],
    ...
    def test_grid_integrate_z(self):  # <<< FIXME
        ...
        np.testing.assert_equal(
            ...
            self.testlocpot2.grid.slice(5, 'z'), z_all)
...
#        np.testing.assert_equal(testarray2D + 12, z1)
    ...
    def test_grid_integrate_y(self):
        ...
        np.testing.assert_equal(
            ...
            self.testlocpot2.grid.slice(3, 'y'), y_all)
    ...
    def test_grid_integrate_x(self):  # <<< FIXME
        ...
        np.testing.assert_equal(
            ...
            self.testlocpot2.grid.slice(2, 'x'), x_all)
    ...
    def test_min_along(self):
        ...
        np.testing.assert_equal(np.array([0., 3., 6., 9.]),
                                self.testlocpot2.grid.min_along_axis('y'))
        np.testing.assert_equal(np.array([0., 12., 24., 36., 48., 60.]),
                                self.testlocpot2.grid.min_along_axis('z'))

    @with_setup(setup=setUp)
    def test_max_along(self):
        """Test for LOCPOT max."""
        np.testing.assert_equal(np.array([69., 70., 71.]),
                                self.testlocpot2.grid.max_along_axis('x'))
        np.testing.assert_equal(np.array([62., 65., 68., 71.]),
                                self.testlocpot2.grid.max_along_axis('y'))
        np.testing.assert_equal(np.array([11., 23., 35., 47., 59., 71.]),
                                self.testlocpot2.grid.max_along_axis('z'))

    @with_setup(setup=setUp)
    def test_median_along(self):
        """Test for LOCPOT median."""
        np.testing.assert_equal(np.array([34.5, 35.5, 36.5]),
